...
from linkman_2d_model import LinkManModel
from linkman_2d_dataset import LinkManDataset
...
def parse_datasets(path):
    ...
    for traj_data in os.listdir(path):
        data_x_path = path + traj_data + "/robot_state.csv"
...
def plot_vel_and_ctrl_distribution(list_of_datasets: list):
    ...
    with torch.no_grad():
        ...
        data_for_visualization = torch.cat(
            [all_state[:, 3:], all_control[:, :]], 1).cpu().numpy()
    ...
    fig, ax = plt.subplots()
    graph = sns.pairplot(
        data_for_visualization[::100], plot_kws={'alpha': 0.1})
...
def main():
    ...
    # define datasets
    train_data = parse_all_datasets(
        config.train_data_path, exclude=config.exclude_train_data.split())
